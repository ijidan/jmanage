...
layui.define(['jquery', 'table'], function(exports) {
	...
	yutons_sug.prototype.render = function(opt) {
		...
		$("#" + opt.id).parent().mouseleave(function(e) {
		...
		if (opt.type == "sugTable") {
			...
			//如果type为sug，则初始化下拉框
			$("#" + opt.id).next().css({"border": "solid #e6e6e6 1px","box-sizing":"border-box", "max-height":"200px","overflow":"auto"});
			/* 输入框鼠标松开事件 */
			$("#" + opt.id).mouseup(function(e) {
				opt.obj = this;
				getSug(opt);
	...
	function getSugTable(opt) {
		...
		table.on('rowDouble(' + "yutons_sug_" + opt.id + ')', function(obj) {
			//获取选中行所传入字段的值
			$("#" + opt.id).trigger('valChange', [obj.data[opt.id]]).val(obj.data[opt.id]);
			$("#" + opt.id).next().hide().html("");
		});
	}
	...
	function getSug(opt) {
		...
		$.ajax({
			...
			success: function(data) {
				let html = "";
				layui.each(data.data.list, (index, item) => {
	...
	let dojob = function() {
		...
		$("#" + sessionStorage.getItem("inputId")).blur();
		$("#" + sessionStorage.getItem("inputId")).trigger('valChange', [value]).val(value);
		$("#" + sessionStorage.getItem("inputId")).next().hide().html("");
	}
	...
	window.getCon = function(obj) {
		let value = $(obj).text();
		$("#" + $(".item").parent().prev().attr("id")).trigger('valChange', [value]).val(value);
